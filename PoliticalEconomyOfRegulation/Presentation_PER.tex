...
\begin{frame}{Texts and Research Focus}
\phantomsection\label{texts-and-research-focus}
\begin{block}{Massoc (2019) : ``Taxing Stock Transfers in the First
Golden Age of Financial Capitalism: Political Salience and the Limits on
the Power of Finance''}
\phantomsection\label{massoc2019-taxing-stock-transfers-in-the-first-golden-age-of-financial-capitalism-political-salience-and-the-limits-on-the-power-of-finance}
\begin{itemize}
\tightlist
\item
...
\begin{block}{Feldmann and Morgan (2021) : ``Brexit and British Business
Elites: Business Power and Noisy Politics''}
\phantomsection\label{feldmann2021-brexit-and-british-business-elites-business-power-and-noisy-politics}
\begin{itemize}
\tightlist
\item
...
Discussion}\label{side-by-side-paper-summaries-and-critical-discussion}
...
\begin{center}
\includegraphics[width=2.94792in,height=\textheight,keepaspectratio]{Screenshot 2025-11-18 at 14.13.03.png}
\end{center}
...
\item
  ...
  whereas Feldmann and Morgan (2021) explain when business can (or
...
\begin{frame}{Who Pulls the Trigger?: Incumbents' Silence (Session 3/4)
vs.~Underdog's Noise (Session 9)}
\phantomsection\label{who-pulls-the-trigger-incumbents-silence-session-34-vs.-underdogs-noise-session-9}
~\textbf{Why Noise: The engine for Democratic Cycling}

\begin{itemize}
\item
  \textbf{Incumbents}: Prefer \emph{silence} (``quiet politics'') to
  \emph{maintain structural/ instrumental power}
\item
  \textbf{Underdog}: Need \emph{noise} (``noisy politics'') to mobilize
  the majority and \emph{challenge established powers}
\end{itemize}

\textbf{→ Noisy Politics as a ``\emph{safety valve}'' for Democracy}
\end{frame}

\begin{frame}{The Paradox of the Underdog's Weapon: Safety Valve or
Epistemic Chaos}
\phantomsection\label{the-paradox-of-the-underdogs-weapon-safety-valve-or-epistemic-chaos}
\tiny

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1933}}
...
(McGann 2006)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\textbf{Epistemic Chaos}

(Culpepper 2021)
\end{minipage} \\
\midrule\noalign{}
\endhead
\textbf{Core Mechanism} & \begin{minipage}[t]{\linewidth}\raggedright
\textbf{Majoritarianism}

Mobilizing the defeated majority\\
to overturn results\\
\strut
\end{minipage} & \textbf{Tribalism}

Parallel universes believing in alternative facts \\
\textbf{Prerequisite Condition} &
\begin{minipage}[t]{\linewidth}\raggedright
\textbf{Shared Public Sphere}

(Consensus on ``What is Fact?'')\\
\strut
\end{minipage} & \textbf{Condition Failed} (Facts denied: ``Fake News'',
``Enough of experts'') \\
\textbf{Outcome} & \begin{minipage}[t]{\linewidth}\raggedright
\textbf{Healthy Democratic Cycling}

Breaking the incumbent cartel

\& Ensuring policy responsiveness\\
\strut
\end{minipage} & \begin{minipage}[t]{\linewidth}\raggedright
\textbf{Polarized Gridlock \& Rupture}

Paralyzing deliberation\\
\& Destructive rupture\strut
\end{minipage} \\
\textbf{Case Application} & \textbf{1905 New York \& 1893 France}
(Massoc 2019)

Shared anger over ``Wall Street Corruption''

\textcolor{mygreen}{(condition met)}

→ Functioned as a \textbf{Safety Valve} & \textbf{2016 Brexit (Feldmann
and Morgan 2021)}

Economic facts denied

\textcolor{red}{(condition failed)}

Plunged into \textbf{Epistemic Chaos} \\
\bottomrule\noalign{}
\end{longtable}
...
\begin{frame}{The Hollow Shot? Winning the War of Votes, Losing the
Battle of Implementation}
\phantomsection\label{the-hollow-shot-winning-the-war-of-votes-losing-the-battle-of-implementation}
\textbf{Depoliticized Complexity (Session 6 \& 8) vs.~Repoliticized
Simplicity (Session 9)}
...
\endhead
& \begin{minipage}[t]{\linewidth}\raggedright
\textbf{Incumbents' Shield}

(Thiemann and Lepoutre 2017)\\
\strut
\end{minipage} & \textbf{Underdogs' Spear}

(Massoc 2019; Feldmann and Morgan 2021) \\
\textbf{Weapon} & \begin{minipage}[t]{\linewidth}\raggedright
\textbf{Complexity}
...
Moral Framing \& Manichean Slogans\\
\strut
\end{minipage} \\
\textbf{Goal} & \begin{minipage}[t]{\linewidth}\raggedright
\textbf{Depoliticization}

Keep regulation in expert domain\\
\strut
\end{minipage} & \textbf{Re}-\textbf{politicization}

Mobilize public anger \\
\textbf{Sources} & \begin{minipage}[t]{\linewidth}\raggedright
\textbf{Conceptual \& Structural Power\\
}\strut
\end{minipage} & \textbf{Noisy Politics \& Political Salience} \\
\textbf{Arena of Dominance} & \textbf{Technical Rule-making \& Loophole}
& \textbf{Public Debate \& Voting} \\
\bottomrule\noalign{}
\end{longtable}
...
\begin{frame}{The Hollow Shot? Winning the War of Votes, Losing the
Battle of Implementation}
\phantomsection\label{the-hollow-shot-winning-the-war-of-votes-losing-the-battle-of-implementation-1}
\footnotesize

\begin{block}{\textbf{After the Strike}}
\phantomsection\label{after-the-strike}
\begin{itemize}
\item
  \textbf{The Shot (Political Win):} Simplification successfully breaks
  the shield → \emph{Law Passed.}
\item
  \textbf{The Trap (Implementation Gap):} Simple moral language fails to
  control technical details during implementation.
\end{itemize}

→ \textbf{Result:} ``The Hollow Shot'' → Political Victory \(\not=\)
Regulatory Success
\end{block}

\begin{block}{\textbf{Evidence: The Hollow Aftermath}}
\phantomsection\label{evidence-the-hollow-aftermath}
\begin{itemize}
\item
  \textbf{The Cunning Recapture:} 1905 New York (Massoc, 2017)\\
  → The STT laws adopted (1905)\\
  → \emph{``special arrangements and exemptions''} via \emph{technical
  lobbying}\textbf{\hfill\break
  }→ \emph{Regulatory Failure} (tax revenues declined)
\item
  \textbf{The Incompetent Chaos}: 2016 Brexit (Feldmann \& Morgan,
  2021)\\
  → Won the Brexit Vote (52\%)\\
  → \emph{``chaotic negotiations''} post-referendum\\
  → \emph{Regulatory Gridlock} (complex trade realities overwhelmed the
  simple mandate)
\end{itemize}
...
\begin{frame}{The Double-Edged Sword of Noise}
\phantomsection\label{the-double-edged-sword-of-noise}
\textbf{The Conditions for Victory (How to Win)}

\begin{itemize}
\tightlist
\item
  \textbf{Business power} is not absolute. It breaks when simple, moral
  framing (Massoc 2019) meets \emph{elite fragmentation} (Feldmann and
  Morgan 2021)
\end{itemize}

\textbf{The Costs of Victory}

\begin{itemize}
\item
  \textbf{Institutional Stability}: Undermining the Enforcement Pyramid
\item
  \textbf{Epistemic Consensus}: Destroying the ``shared facts''
\item
  ~\textbf{Regulatory Implementation}: Ignoring the technical complexity
\end{itemize}
\end{frame}

\begin{frame}{The Path Forward}
\phantomsection\label{the-path-forward}
\begin{itemize}
\item
  Noisy politics is a powerful \emph{shock treatment} to break a ``Quiet
  Politics'' deadlock\ldots{}
\item
  \ldots but it may not be a \emph{sustainable solution} for long-term
  regulatory governance
\end{itemize}

\begin{block}{}
\phantomsection\label{section}
→ True change requires using the \emph{noise} to \emph{open the door,}\\
then building \textbf{new} \textbf{consensus} and
\textbf{counter}-\textbf{expertise} to walk through it.
\end{block}
\end{frame}
...
\begin{itemize}
\item
  How should we explain phenomena like \emph{``woke capitalism''} (e.g.,
  ESG, PC issues), where incumbents actively generate noise or leverage
  social issues for their benefit? Is this an evolution of Massoc's
  (2017) framing strategy?
\item
  Could new models like \emph{citizens' assemblies} or
  \emph{deliberative polling} be the alternative -- safely bringing
  ``public noise'' inside the institutional framework for deliberation?
\item
  If \emph{simplification} is a prerequisite for political victory, how
  should complex technical regulations (e.g., AI, Climate Finance) be
  communicated to the public?
\item
  Should regulatory experts: (a) retreat deeper into \emph{quiet
  politics} to avoid political attacks, or (b) jump into the center of
  noisy politics and actively engage in public persuasion?
\item
  Feldmann and Morgan (2021) focused on the UK, a liberal market
  economy. Would noisy politics have the same destructive power in
  coordinated market economies like Germany or Japan?
\end{itemize}
...
\bibitem[\citeproctext]{ref-mcgann2006}
McGann, Anthony J. 2006. \emph{The Logic of Democracy: Reconciling
Equality, Deliberation, and Minority Protection}. University of Michigan
Press.

\bibitem[\citeproctext]{ref-thiemann2017}
Thiemann, Matthias, and Jan Lepoutre. 2017. {``Stitched on the Edge:
Rule Evasion, Embedded Regulators, and the Evolution of Markets.''}
\emph{American Journal of Sociology} 122 (6): 1775--1821.
\url{https://doi.org/10.1086/691348}.
